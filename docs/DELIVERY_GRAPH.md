...
flowchart LR
  ...
  builder --> packaging["Sandbox packaging checks"]
  packaging -->|pass| test["Test"]
  packaging -->|pass| reviewer["Reviewer"]
  packaging -->|fail| builder
  test --> gate["Gate"]
  reviewer --> gate
  security --> gate
...
The project workflow creates a private Forgejo repository, the **Orchestra delivery** project board, and an iteration umbrella issue before agent work starts. Forgejo bootstrap creates one local user per delivery role (`orchestra-manager`, `orchestra-builder`, …) plus the admin automation account; each repository grants those users write collaborator access, and agent issue comments, creates, and label changes authenticate as that role’s user. After Manager completes, Orchestra materializes the first wave of meaningful Forgejo work issues (from Manager `workPackages`, or a safe default set) with `agent/{role}` assignment labels. Planner is the primary splitter; every role may also `createIssue` when required. Agents load open issues labeled for them into their orders, claim work by keeping `agent/{role}` and `item/in-progress`, post comments, edit titles and bodies, call collaborators by adding other `agent/*` labels, and remove their label when done (`completeIssue`). Human answers and agent comments are mirrored onto those issues. The Temporal delivery graph still decides when a role may run; Forgejo issues decide what they collaborate on.

The workflow calculates every dependency-ready wave and submits bounded, authority-scoped commands to all ready role actors together. Each actor receives explicit repository coordinates, the newest relevant artifacts from prior iterations, its own prior output, the current dependency handoffs, and its assigned Forgejo work. Builder also receives earlier `source-file:*` outputs so revision work remains incremental. Generated source cannot write reserved CI/workflow trees or root credential/environment files. Handoffs are recorded in project history and Forgejo. Candidate-producing outputs are committed to the shared `iteration-N-agents` branch; Test, Reviewer, and Gate assurance outputs are ledger-only and carry the immutable candidate `sourceRevision`. A Gate result opens a pull request only when its structured decision is `pass`, its missing-evidence list is empty, and every declared upstream evidence artifact is present for that frozen revision. Manager then records a separate ledger-derived rationale and recommendation before human review opens. Otherwise the iteration blocks and reactivates the affected downstream path after remediation; neither Manager nor the UI can silently bypass Gate.
...
UX output can include a structured user journey. The model worker converts that structure into an escaped, script-free SVG rather than trusting model-authored markup. Planner also emits a structured `packaging-plan` limited to allowlisted checks (`docker_build`, `container_health`, `unit_tests`). Orchestra materializes a fixed Forgejo Actions workflow at `.forgejo/workflows/iteration-packaging.yml` from that plan; agents cannot free-write workflow YAML. Builder must supply the versioned preview runtime contract: a root Dockerfile, one exposed port at `8080`, and an active `GET /health` check. Before handoff, Builder runs a plan→code→sandbox loop against preview-manager `POST /build-checks` until required packaging checks pass (or the iteration blocks for resume). Results are recorded as `packaging-evidence` and Forgejo commit statuses. Before Test runs, the validation worker asks the local preview manager to resolve the iteration branch to an immutable Forgejo revision, reuse a sandbox-built image when the revision tag already exists, start it in the isolated preview network, and prove health. A deployment failure creates a visible blocker and the workflow waits for a retry; Test, Gate, and the human approval checkpoint cannot bypass it.
